...
template <class T> bool Tree<T>::isEmpty() const { return _first == nullptr; }
template <class T> void Tree<T>::add(T value) {
  if (isEmpty()) {
...
*/
template <class T> bool Tree<T>::isIn(T value, Element<T> *current) const {
  while (current and (current->getValue() != value)) {
...
template <class T> Element<T> *Tree<T>::remove(Element<T> *current, T value) {
  if (current == nullptr)
    return nullptr;
...
template <class T> Element<T> *Tree<T>::treeMinValue(Element<T> *current) {
...
template <class T> void Tree<T>::toDisp(int a) {
  switch (a) {
  case 0:
    dispPreorder(_first);
    break;
...
template <class T> void Tree<T>::dispPreorder(Element<T> *current) {
  if (current) {
...
template <class T> void Tree<T>::dispInorder(Element<T> *current) {
  if (current) {
...
template <class T> void Tree<T>::dispPostorder(Element<T> *current) {
  if (current) {
...
template <class T> void Tree<T>::setDepth(Element<T> *current) {
  // Element<T>*current = _first;
  if (current) {
    if (current->_left != nullptr) {
...
template <class T> unsigned int Tree<T>::minPath() {
...
template <class T> unsigned int Tree<T>::maxPath() {
  Element<T> *current = _first;
